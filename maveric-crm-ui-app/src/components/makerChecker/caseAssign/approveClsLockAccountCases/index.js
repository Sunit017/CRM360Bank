...
import { approveClsLockCasesCol } from "../../common/columnDef";
...
const ApproveClsLockAccountCases = () => {
  ...
  const CellCaseIdRender = (params) => {
    const gotoCaseIdView = (value, data) => {
      dispatch(
        addOps360PageDetails({ pageId: "approveCloseLockUnlockAccount", data })
      );
    };
  ...
  const columnsDef = approveClsLockCasesCol.map((item) => {
    const col = { ...item };
  ...
  const onExportClick = () => {
    ...
    gridApi.exportDataAsCsv({
      fileName: `approveCloseLockUnlockAccountCases_${todayDate}`,
  ...
  return (
    <>
      <div style={{ fontWeight: 700, textAlign: "center" }}>
        Approve / Close / Lock / Unlock Account Cases
      </div>
      <br />
...
export default ApproveClsLockAccountCases;
