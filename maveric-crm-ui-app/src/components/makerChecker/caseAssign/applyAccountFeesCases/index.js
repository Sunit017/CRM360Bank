...
import { applyAccountFeeCasesCol } from "../../common/columnDef";
...
const ApplyAccountFeesCases = () => {
  ...
  const caseAssignData = [
    ...
    {
      caseId: "AC123660",
      productName: "Term Deposit - Monthly",
  ...
  const CellCaseIdRender = (params) => {
    const gotoCaseIdView = (value, data) => {
      dispatch(addOps360PageDetails({ pageId: "applyAccountFees", data }));
    };
  ...
  const columnsDef = applyAccountFeeCasesCol.map((item) => {
    const col = { ...item };
  ...
  const onExportClick = () => {
    ...
    gridApi.exportDataAsCsv({
      fileName: `applyAccountFeesCases_${todayDate}`,
...
export default ApplyAccountFeesCases;
